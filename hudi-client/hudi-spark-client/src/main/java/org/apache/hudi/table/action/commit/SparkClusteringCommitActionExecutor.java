...
public class SparkClusteringCommitActionExecutor<T extends HoodieRecordPayload<T>>
    extends BaseSparkCommitActionExecutor<T> {

  private static final Logger LOG = LogManager.getLogger(SparkClusteringCommitActionExecutor.class);

  public SparkClusteringCommitActionExecutor(HoodieEngineContext context,
                                             HoodieWriteConfig config, HoodieTable table,
                                             String instantTime, HoodieClusteringPlan plan) {
    super(context, config, table, instantTime, WriteOperationType.CLUSTERING);
  ...
  protected Partitioner getPartitioner(WorkloadProfile profile) {
    return new BulkInsertPartitioner<>(profile, context, table, config);
  }
